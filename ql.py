...
if __name__ == "__main__":
    ...
    for episode in tqdm(range(episodes)):
        ...
        mycars = [10,11,12,13]  
        targetVehicle = "car_12" # our car to control

        # we control it now
        traci.vehicle.setSpeedMode(vehID=targetVehicle, sm=0)
        
        traci.vehicle.setSpeed(targetVehicle,random.randint(0,20)) # random speed for starting
    ...
    # now we do a last "episode" to see the results in the GUI
    sumoBinary = "C:/Program Files (x86)/Eclipse/Sumo/bin/sumo-gui.exe"
    traci.start([sumoBinary, "-c", "./data/demo.sumocfg", "--no-step-log", '-W'])
    ...
    # we save the qtable
    with open('qtableQL.npy', 'wb') as f:
        np.save(f, q_table)
